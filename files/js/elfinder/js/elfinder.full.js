			...
			var id = 'el-finder-io-'+(new Date().getTime()),
				...
				d.append(f.append(e.hide()).prepend(m).append(b)).dialog({
						...
						buttons     : {
							Cancel : function() { $(this).dialog('close'); },
							Ok     : function() {
								var check=false;$(':file', f).each(function(){if(this.value!='') check=true;});
...
elFinder.prototype.quickLook = function(fm, el) {
	...
	this.win     = $('<div class="el-finder-ql"/>').hide()
		.append($('<div class="el-finder-ql-drag-handle"/>').append($('<span class="ui-icon ui-icon-circle-close"/>').click(function() { self.hide();/* $(document).unbind($.browser.opera ? 'keypress' : 'keydown');*/ })).append(this.title))
		.append(this.ico)
		.append(this.media)
	...
	this.init = function() {
		...
		if (this.fm.options.places) {
			...
			this.fm.view.plc.droppable({
				accept    : 'div.directory',//(div,tr).directory
				tolerance : 'pointer',
