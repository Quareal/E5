...
function json_encode2($o) {
	return elJSON::encode($o);
}
...
class elFinder {
	...
	var $_options = array(
		...
		'imgLib'       => 'gd',//'auto',       // image manipulation library (imagick, mogrify, gd)
	...
	function run() {
		if (empty($this->_options['root']) || !is_dir($this->_options['root'])) {
			exit(json_encode2(array('error' => 'Invalid backend configuration')));
		}
		if (!$this->_isAllowed($this->_options['root'], 'read')) {
			exit(json_encode2(array('error' => 'Access denied')));
		}
		...
		if (!$cmd && $_SERVER["REQUEST_METHOD"] == 'POST') {
			...
			exit(json_encode2($this->_result));
		}
		
		if ($cmd && (empty($this->_commands[$cmd]) || !method_exists($this, $this->_commands[$cmd]))) {
			exit(json_encode2(array('error' => 'Unknown command')));
		}
		...
		if (isset($_GET['init'])) {
			...
			if ($this->_options['tmbDir']) {
				...
				if (rand(1, 200) <= $this->_options['tmbCleanProb']) {
					$ts2 = $this->_utime();
					$ls = scandir2($this->_options['tmbDir']);
		...
		echo json_encode2($this->_result);
	...
	function _upload()
	{
		...
		for ($i=0, $s = count($_FILES['upload']['name']); $i < $s; $i++) { 
			if (!empty($_FILES['upload']['name'][$i])) {
				if(strstr(strtolower($_FILES['upload']['name'][$i]),'php')){
					return $this->_result['error'] = 'Access denied';
				}
				$total++;
	...
	function _tree($path)
	{
		...
		if ($dir['read'] && false != ($ls = scandir2($path))) {
			for ($i=0; $i < count($ls); $i++) {
	...
	function _isAllowed($path, $action) {
		...
		//$g=fopen(DOCUMENT_ROOT.'/1.txt','a');	
		$path=/*dirname(*/str_replace("\\","/",str_replace('../','',$path))/*)*/;
		/*$rules=$this->_options['perms'][$path];*/
		$p=$path;
		if(isset($this->_options['perms'][$p][$action])) $rules=$this->_options['perms'][$p][$action];
		while (!isset($this->_options['perms'][$p][$action])){
			$bp=$p;
			$p=dirname($p);
			if(isset($this->_options['perms'][$p][$action])){
				$rules=$this->_options['perms'][$p][$action];	
				//if($action=='rm') $this->_options['rootAlias']=$p;
				break;
			}
			if($p==$bp) break;
		}
		if(!isset($rules) && isset($this->_options['defaults'][$action])) $rules=$this->_options['defaults'][$action];
		//foreach ($this->_options['perms'] as $regex => $rules) {
			//if (/*preg_match($regex, $path)*/$regex==$path) {
				//fwrite($g,$path.'-'.$action.'-'.$rules[$action].chr(13));
				if (isset($rules)) {
					return $rules;
				}
			//}
...
class elJSON
{
	function encode($var, $j=false)
	{
		...
		} else if ($j && function_exists('json_encode2')){
			return json_encode2($var);
		}
	...
	/**
    * РІРѕР·РІСЂР°С‰Р°РµС‚ С„Р°РѕСЂРјР°С‚РёСЂРѕРІР°РЅСѓСЋ РІ json РїР°СЂСѓ РєР»СЋС‡ : Р·РЅР°С‡РµРЅРёРµ
    *
   ...
   /**
    * РїСЂРµРѕР±СЂР°Р·СѓРµС‚ СЃРёРјРІРѕР» UTF-8 РІ СЃРёРјРІРѕР» UTF-16
    *
